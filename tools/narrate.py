...
SCENES = [
    ...
    ("s5", """The judged week closed the books. Two hundred and four income candidates were evaluated, and every single one was refused. Zero trades opened, zero fills, no win rate exists — the refusal log itself is the deliverable. One deliberate exception: the evening before the jobs report, a one-day straddle on SPY was priced at zero point five percent of spot against a zero point nine percent gate. Cheap against the median jobs-morning move, so the agent bought five contracts at three ninety three, one point nine seven percent of equity, logged before placement. That is the whole week: refusal as a system, plus one event it could not refuse.""",),

    ("s5b", """I said at the start this would not win on profit and loss, and it will not. What it can do is account for itself. Every position sized from live equity, every refusal written down with a reason, every decision hash-chained so the record cannot be edited after the fact. A field this large will contain many accounts up fifteen percent on one lucky bet, and approximately none that can explain themselves. That is the entry. The repository, the write-up and this video are disclosed as AI-assisted.""",),
]
